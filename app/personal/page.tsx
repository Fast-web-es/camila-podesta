      ...
      <section className="relative">
              ...
              {projects.map((project, index) => {
                ...
                return (
                  <React.Fragment key={project.id}>
                    <Link 
                      href={`/personal/${project.id}`}
                      className="relative group inline-block"
                      onMouseEnter={() => isDesktop && setActiveProject(project)}
                      onMouseLeave={() => isDesktop && setActiveProject(null)}
                    >
                      <h2 className={`
         ...
         {isDesktop && (
           <motion.div 
              ...
              style={{ x, y, translateX: '-50%', translateY: '-50%', width: '500px' }}
           >
              <AnimatePresence mode="wait">
                {activeProject && (
                  ...
                  >
                    <img src={activeProject.thumbnail} alt={activeProject.title} className="w-full h-auto object-cover aspect-[4/3]" />
                  </motion.div>
                )}
              </AnimatePresence>
           </motion.div>
         ...
         <div className="lg:hidden flex flex-col space-y-16 pb-20">
            {projects.map((project) => (
              <Link href={`/personal/${project.id}`} key={project.id} className="group block">
